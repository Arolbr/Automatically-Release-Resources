...
作为习惯于 **Vue2** 和 **SpringBoot** 工程化开发的开发者，你可以通过以下方式扩展本项目：

* **GUI 增强**：使用 Web 技术栈通过 Electron 或 WebView2 为其添加一个 Vue2 编写的监控仪表盘。
* **配置工程化**：仿照 SpringBoot 的 `application.yml`，为本项目添加一个外部 JSON 配置文件，动态管理 `blacklist` 和 `threshold`。
* **日志系统**：引入轻量级的日志记录，监控每次内存优化的具体效果。
